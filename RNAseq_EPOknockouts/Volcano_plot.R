### Function for Volcano Plot during differential Gene Expression Analysis ###
### Charli (Stoneman) Harlow ###

VolcanoPlot <- function(toptable, NominalCutoff, AdjustedCutoff, LabellingCutoff, FCCutoff, main)
{
	...
	plot <- ggplot(toptable, aes(x=log2FoldChange, y=-log10(padj))) +
	  ...
	  #scale_y_continuous(breaks=seq(0,250,by=50), lim=c(0,250)) +
		...
		geom_text(data=subset(toptable, padj<LabellingCutoff & abs(log2FoldChange)>FCCutoff),
			aes(label=subset(toptable, padj<LabellingCutoff & abs(log2FoldChange)>FCCutoff)$genenames),
			size=2.25,
		...
		geom_hline(yintercept=-log10(NominalCutoff), linetype="longdash", colour="black", size=0.4)
}
